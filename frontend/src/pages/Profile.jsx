      ...
      try {
        ...
        if (userData) {
          ...
          const dummyUser = {
            ...
            hiringNeeds: '',
            timezone: '',
            skills: ['React', 'JavaScript', 'Node.js', 'UI/UX Design', 'Tailwind CSS'],
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
        <div className="max-w-md w-full bg-white/10 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-white/20">
          <div className="text-red-300 text-center font-medium text-lg">{error}</div>
          <button 
          ...
          <div className="pt-20 px-8 pb-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
              <div className="flex-1">
                <h1 className="text-3xl font-bold text-white mb-2">{user.name || 'Not provided'}</h1>
                <p className="text-purple-200 text-lg mb-1">{user.email}</p>
                <p className="text-purple-200 mb-4">{user.phone || 'No phone number'}</p>
                
                {/* Stats */}
                <div className="flex space-x-6 mb-4">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white">{user.stats?.projects || 0}</div>
                    <div className="text-purple-200 text-sm">Projects</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white">{user.stats?.meetings || 0}</div>
                    <div className="text-purple-200 text-sm">Meetings</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-white">{user.stats?.connections || 0}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-purple-300 text-sm font-medium">Email Address</label>
                    <p className="text-white text-lg font-semibold">{user.email}</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="text-purple-300 text-sm font-medium">Phone Number</label>
                    <p className="text-white text-lg font-semibold">{user.phone || 'Not provided'}</p>
                  </div>
                  <div>
                    <label className="text-purple-300 text-sm font-medium">Location</label>
                    <p className="text-white text-lg font-semibold">{user.location || 'Not specified'}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-purple-300 text-sm font-medium">Hiring Needs</label>
                  <p className="text-white text-lg font-semibold">
                    {user.hiringNeeds || 'Not specified'}
                  </p>
                </div>
                <div>
                  <label className="text-purple-300 text-sm font-medium">Timezone</label>
                  <p className="text-white text-lg font-semibold">
                    {user.timezone || 'Not specified'}
                  </p>
                </div>
                <div>
                  <label className="text-purple-300 text-sm font-medium">Availability</label>
              ...
              <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-6">
                ...
                <div className="space-y-3">
                  {user.socialLinks.github && (
                    <a href={`https://github.com/${user.socialLinks.github}`} className="flex items-center space-x-3 text-purple-200 hover:text-white transition-colors duration-300">
                  ...
                  {user.socialLinks.linkedin && (
                    <a href={`https://linkedin.com/in/${user.socialLinks.linkedin}`} className="flex items-center space-x-3 text-purple-200 hover:text-white transition-colors duration-300">
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
